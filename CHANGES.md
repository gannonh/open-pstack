...
The startup rule routes non-trivial work into `poteto-mode`, resolves `pstack:<name>` references to the unprefixed plugin skills Cursor exposes, and points at two references. `references/cursor-tools.md` is the Cursor counterpart of `codex-tools.md`: skill invocation, `Task` subagents with `model` and `run_in_background`, `TodoWrite`, `AskQuestion` versus plain text in CLI print mode, background `Shell` plus `AwaitShell`, the `run`/`verify`/`create-skill`/`loop` built-in substitutions, transcript paths, and the instructions file. `provider-dispatch.md` gains the Cursor parent row: `cursor:*` descriptors run natively through `Task` with the catalog's composed `<selector>-<effort>` id, while `claude:*`, `codex:*`, and `grok:*` run through the existing external runner with `--parent cursor`. The runner accepts `cursor` as a parent, rejects `--provider cursor` from that parent as same-provider recursion, and strips Cursor's session markers from non-Cursor children the way it already strips Claude and Codex markers.

`setup-pstack` writes `~/.cursor/rules/open-pstack-models.mdc` on a Cursor parent: an `alwaysApply: true` frontmatter block followed by the shared sheet. Cursor loads user rules itself, so the rule is both sheet and integration. The shared parser reads the frontmatter as preamble, so a hand-edited descriptor keeps its provider, model, and effort, and an unchanged rerun renders byte-identical content. Setup never reads, migrates, overwrites, or deletes the original plugin's `pstack-models.mdc`, and the documentation advises disabling the original plugin while the fork is enabled. Cursor may replace a subagent's configured model under team or plan restrictions; the dispatch reference treats an observed mismatch as a dropout and never accepts a model's self-report as evidence. No implicit timeout, weaker-model fallback, provider substitution, or compatibility layer was added.

Observed with Cursor CLI 2026.09.02 on Linux: `--plugin-dir` applies a local plugin's skills and agents (an agent left out of the explicit `agents` list stays hidden) but not its rules; plugin rules apply to installed plugins. A plugin directory inside the running workspace loses its skills, so point `--plugin-dir` outside the workspace. `~/.cursor/rules/*.mdc` loads when the workspace is under the home directory. The `Task` tool accepts a short server-curated list of subagent model slugs that differs from `cursor-agent models` and varies between sessions; a cataloged id missing from it is an `unavailable-model` dropout for the native lane. The CLI has no `plugin install` subcommand; `plugin marketplace add|list|remove|update` are the CLI's plugin commands and installation happens in the IDE.
...
The first catalog revision keeps every 1.3.1 descriptor (`claude:fable`, `claude:opus`, `codex:gpt-5.6-sol`, `cursor:cursor-grok-4.6`, standalone `grok:grok-4.6`, plus `inherit-parent` / `auto`) and adds Cursor Fable 5.1 as `cursor:claude-fable-5-1` with efforts through `max`. Claude Fable 5.1 stays the rolling `fable` selector; display names and provider selectors are separate fields. An operator can assign either Fable offering to any eligible role without changing plugin code.

`setup-pstack` enumerates catalog offerings, edits named roles or panel lanes (`how critics[3]`), and probes the exact unique descriptors that will be written. It no longer asks four family questions or enforces one effort per family. A valid hand-edited catalog descriptor is preserved verbatim. Uncataloged predecessor Claude version pins still migrate to the rolling alias; a cataloged explicit version is not rewritten.

The runner requires a cataloged `(provider, selector, effort)` tuple and composes Cursor CLI ids from the offering's `selectorComposition`. Missing CLI, quota, unavailable model, or invalid selection remains a receipt-bearing dropout. There is no automatic provider fallback, implicit timeout, or weaker-model substitute.

Operator guide: [docs/models.md](docs/models.md).
...
**Maintenance.** The open-pstack version string lives in `plugins/pstack/.claude-plugin/plugin.json`, `plugins/pstack/.codex-plugin/plugin.json`, `plugins/pstack/.cursor-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.cursor-plugin/marketplace.json`, and the current-version row in `UPSTREAM.md`. A version bump must update all six. `tests/skill-collision-repro.sh` checks that they match, and it checks that the Cursor manifest keeps the `open-pstack` identifier, the explicit `skills`, `agents`, `rules`, and `hooks` selections, and no Claude-native agent or hook. `.agents/plugins/marketplace.json` carries no version field. The canonical model offerings and first-run role map live in `plugins/pstack/catalog/models.json` and `plugins/pstack/catalog/role-defaults.json`. Workflow skills must not copy those descriptors; the static test greps for leaked slugs. After adding a Claude offering, regenerate native agents with `bun plugins/pstack/skills/poteto-mode/scripts/runner/generate-native-agents.ts`. After a sync that touches `skills/poteto-mode/scripts/`, run `bun install --frozen-lockfile`, `bun run test`, and `bun run typecheck` from that directory. `hooks/session-start-context.md` restates skill one-liners. Re-verify it whenever skill names or descriptions change. The package must not contain a `commands/` layer. Claude Code and Codex load the native `skills/` tree directly, and a command layer duplicates that inventory. The 21 `principle-*` leaves carry `user-invocable: false` to request exclusion from the user picker while `poteto-mode` reads them by path. Claude honors the metadata; Codex 0.149.0 currently does not ([#8](https://github.com/ericlitman/open-pstack/issues/8)). They must not carry `disable-model-invocation`, which would make them unreachable to the model. Re-run the behavioral mode of `tests/skill-collision-repro.sh` after Claude Code upgrades to check both model-initiated and user-initiated native skill invocation.
