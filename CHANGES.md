...
Observed with Cursor CLI 2026.09.02 on Linux: `--plugin-dir` applies a local plugin's skills and agents (an agent left out of the explicit `agents` list stays hidden) but not its rules; plugin rules apply to installed plugins. `~/.cursor/rules/*.mdc` loads when the workspace is under the home directory. The CLI has no `plugin install` subcommand; `plugin marketplace add|list|remove|update` are the CLI's plugin commands and installation happens in the IDE.
