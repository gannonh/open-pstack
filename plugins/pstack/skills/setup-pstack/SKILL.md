...
For each unique descriptor, use the parent route table in `provider-dispatch.md`. On a Claude parent, a `claude:*` probe is a one-turn run of `pstack-<nativeAgentStem>-<effort>`. On a Codex parent, a `codex:*` probe is native `spawn_agent` with the selected `reasoning_effort`. On a Cursor parent, a `cursor:*` probe calls `resolveCursorDescriptorRoute` with this session's `Task` allowlist. Native `Task` uses `route.taskSlug`. An allowlist miss uses `pstack-runner --parent cursor --provider cursor` with the catalog selector and effort (`route.composedCliId` on argv). Record `formatCursorTaskAllowlistError` when the mapped slug is absent. A failed external preflight or a rejected native dispatch that you already selected is a failed probe. Every other pair uses the external runner with the catalog selector and selected effort. Never call the external launcher for Claude or Codex as the parent's own provider.
