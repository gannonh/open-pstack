...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation through provider dispatch using your configured perf-issue descriptor from the model sheet (or the `perf-issue` role in `catalog/role-defaults.json` when the sheet is absent) with `isolated-write` in a dedicated worktree; review the diff. Capture a post-fix trace.
