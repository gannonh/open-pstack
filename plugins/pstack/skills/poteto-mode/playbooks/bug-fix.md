...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation through provider dispatch using your configured bug-fix descriptor from the model sheet (or the `bug-fix` role in `catalog/role-defaults.json` when the sheet is absent) with `isolated-write`, a dedicated worktree, and a specific scope; review the diff.
