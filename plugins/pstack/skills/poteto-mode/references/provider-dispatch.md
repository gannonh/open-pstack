...
| Cursor | external runner | external runner | external runner | `resolveCursorDescriptorRoute` |

Claude and Codex are native exactly when they are the parent's own provider. Cursor `cursor:*` under a Cursor parent uses `resolveCursorDescriptorRoute` in `catalog.ts`. Native `Task` runs when the mapped Task slug is an exact token on that session's `Task` allowlist. Otherwise the parent calls `pstack-runner --parent cursor --provider cursor` with the catalog selector and effort. The configured catalog selector must survive the route. A hard-coded native-agent default must not override it. Provider-specific transformations are catalog/adapter behavior (Cursor effort suffix, Cursor Task slug infix, Claude effort flag), not model substitutions.
...
- Cursor: call `resolveCursorDescriptorRoute` with the parent, offering, effort, and the `Task` tool's current allowed-model list. Setup probes and panel lanes use that same call. Native `Task` uses `route.taskSlug` (`run_in_background: true`, complete task, grounding paths, access mode, and unique output location in the prompt). The mapped slug is the native availability proof. `cursor-agent models` proves the composed CLI id for the external path. When the mapped slug is absent, call the external runner with `--parent cursor --provider cursor` and the catalog selector plus effort. The runner still pins `--model` to `composedCliModel`, never to a Task slug. A `-fast` neighbour is never a native or external substitute. If you selected native `Task` and the tool still rejects, fail closed with `formatCursorTaskAllowlistError` (composed id, mapped slug, allowlist). Do not then pick another Task slug. Cursor may replace a configured subagent model under account or administrator restrictions, so compare the exposed execution evidence with the requested id after the lane ends; a mismatch is a dropout, and a model's self-report is not evidence. When Cursor exposes no served-model field, record `modelEvidence: "pinned-dispatch"` (the exact slug was accepted at dispatch), never `provider-report`; panel synthesis does not treat that as verified completion because silent substitution remains possible. Use an isolated worktree for a writer. Tool names resolve through [`cursor-tools.md`](cursor-tools.md).

Do not send a Claude or Codex same-provider descriptor to the external runner. Those parents still reject `parent === provider`. A Cursor parent may send `cursor:*` to the runner when `resolveCursorDescriptorRoute` returns `external-cursor-agent`.
...
3. Either `modelVerified: true` with `modelEvidence: "provider-report"`, or a Codex receipt with `reportedModel: null`, `modelVerified: false`, and `modelEvidence: "pinned-argv"`. For Claude rolling aliases, the concrete provider report must belong to the requested family. Codex 0.149.0 accepts the exact `--model` argument but does not report the served model in its JSONL stream. A native Cursor `Task` lane records `modelEvidence: "pinned-dispatch"` when no served-model field is exposed, but that does not satisfy this criterion: Cursor may silently substitute the configured model under account restrictions, so panel synthesis treats an unverifiable native Cursor lane as a dropout unless `modelVerified: true` with a matching `provider-report`.
