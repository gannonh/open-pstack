...
A `cursor:<selector>@<effort>` descriptor in a Cursor parent uses `resolveCursorDescriptorRoute` from `catalog.ts`. Pass the parent, offering, effort, and the `Task` tool's current allowed-model list. Setup probes and later panel lanes share that call.

When the route is `native-task`, call `Task` with `model` set to `route.taskSlug`. That slug is not always the composed CLI id. `cursor:cursor-grok-4.6@xhigh` maps to `cursor-grok-4.6-xhigh`. `cursor:claude-fable-5-1@xhigh` maps to `claude-fable-5-1-thinking-xhigh`. Use `subagent_type: "generalPurpose"` unless the calling skill names one, `run_in_background: true`, and put the complete task, grounding paths, access mode, and unique output location in the prompt. Retain the task handle and drain it only after fan-out.

When the route is `external-cursor-agent`, invoke `pstack-runner --parent cursor --provider cursor` with the catalog selector and effort. Argv still pins `route.composedCliId` (`cursor-grok-4.6-xhigh`, `claude-fable-5-1-xhigh`). Record `formatCursorTaskAllowlistError` so the composed id, mapped slug, and allowlist are visible. Never pick a listed neighbour such as a `-fast` variant. If you already selected native `Task` and the tool rejects that exact `taskSlug`, fail closed with that error and do not retry a different Task slug.
...
Do not replace every configured entry with a Cursor model. `/setup-pstack` writes portable `provider:model@effort` descriptors from the catalog to `~/.cursor/rules/open-pstack-models.mdc`. In a Cursor parent, `cursor:*` uses `resolveCursorDescriptorRoute`. Route `claude:*`, `codex:*`, and `grok:*` descriptors through the external launcher with `--parent cursor`, exactly as `provider-dispatch.md` specifies. The launcher accepts `--provider cursor` from a Cursor parent only as the Task-allowlist fallback for the same cataloged offering. The catalog default panel keeps multi-provider frontier diversity; do not substitute a Cursor model for a missing external lane.
