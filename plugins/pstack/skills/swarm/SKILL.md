...
4. Pick the worker descriptor from `swarm workers` in the current harness's pstack model sheet when present. Otherwise use the `swarm workers` role from `catalog/role-defaults.json`. For a model race, name each arm's descriptor up front.
