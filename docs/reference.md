...
- **Routing.** In a Cursor parent, `cursor:*` descriptors call `resolveCursorDescriptorRoute`. Native `Task` uses the mapped slug (`cursor-grok-4.6-xhigh`, `claude-fable-5-1-thinking-xhigh`) when that token is on the session allowlist. Otherwise the parent runs `pstack-runner --parent cursor --provider cursor` with the composed CLI id (`cursor-grok-4.6-xhigh`, `claude-fable-5-1-xhigh`). `claude:*`, `codex:*`, and `grok:*` still run through `pstack-runner --parent cursor`. `inherit-parent` and `auto` omit `model` and inherit the parent's tools, including MCP servers. Cursor can replace a subagent's configured model under team or plan restrictions, so a native lane's exposed execution evidence is compared with the requested id and a mismatch is a dropout. A model's self-report is not evidence.
- **Task allowlist versus `cursor-agent models`.** The CLI's `Task` tool accepts a short, server-curated list of subagent model slugs that differs from `cursor-agent models` and varies between sessions. `nativeTaskSlug` maps catalog composed ids onto that list. Tested rows: `cursor-grok-4.6-xhigh` stays `cursor-grok-4.6-xhigh`; `claude-fable-5-1-high` becomes `claude-fable-5-1-thinking-high`; `claude-fable-5-1-xhigh` becomes `claude-fable-5-1-thinking-xhigh`. A mapped slug that is absent from the allowlist is not a native success. The parent then uses print-mode `cursor-agent` with the unmapped composed id. A `-fast` neighbour is never counted as the requested model. If native `Task` was selected and still rejects, `formatCursorTaskAllowlistError` names the composed id, the mapped slug, and the allowlist. External verification kebab-cases the init display name, drops context-window and thinking-mode tokens, maps `extra-high` to `xhigh`, and requires exact equality with the composed CLI id.
...
| Multi-model panels (arena, architect, interrogate) | Catalog role defaults restore the upstream frontier panel. Same-provider lanes stay native; external lanes use the bundled runner. Alternate cataloged offerings (for example Cursor Fable 5.1) may replace a lane without a skill edit. |
