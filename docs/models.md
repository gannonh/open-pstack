...
External `cursor-agent` verification compares that composed CLI id to the kebab-cased init display name. `normalizeCursorModelId` drops hyphen-delimited context-window tokens (`300k`, `1m`) and thinking-mode tokens (`thinking`, or `no` immediately followed by `thinking`), maps known effort aliases (`extra-high` to `xhigh`), and then requires exact equality. Identity is the family stem, the catalog effort, and an optional `-fast` suffix. Unknown extra tokens, a wrong effort, and a `-fast` neighbour fail closed. The listing display on `cursor-agent models` is not the match key, because live init names can carry extra tokens the listing omits.
